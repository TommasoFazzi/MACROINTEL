      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            {messages.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <h2 className="text-xl font-semibold text-white/80 mb-2">
                  Interroga l&apos;Intelligence Database
                </h2>
                <p className="text-gray-400 text-sm max-w-md">
                  Fai domande su eventi geopolitici, trend, segnali di mercato e storyline.
                  Oracle 2.0 usa strumenti multipli per darti risposte fondate.
                </p>
                <div className="mt-6 grid grid-cols-2 gap-2 max-w-lg">
                  {[
                    'Cosa è successo a Taiwan questa settimana?',
                    'Quanti articoli sulla Cina negli ultimi 30 giorni?',
                    'Quali sono i segnali di trading più forti?',
                    'Come si è evoluta la narrativa sull\'Ucraina?',
                  ].map((suggestion) => (
                    <button
                      key={suggestion}
          ...
          <div className="border-t border-white/10 px-6 py-4">
            <div className="flex gap-3 items-end">
              <textarea
                ...
                placeholder="Fai una domanda di intelligence... (Enter per inviare, Shift+Enter per andare a capo)"
                rows={2}
        ...
        <div className="w-80 border-l border-white/10 flex flex-col overflow-hidden hidden md:flex">
          <div className="px-4 py-3 border-b border-white/10">
            <h3 className="text-sm font-semibold text-white/70">Fonti & Analisi</h3>
          </div>
          <div className="flex-1 overflow-y-auto px-4 py-3">
              ...
              <div className="mb-4">
                ...
                {typeof lastAssistantMessage.metadata?.execution_time === 'number' && (
                  <div className="text-xs text-gray-500 mt-1">
                    Eseguito in {(lastAssistantMessage.metadata.execution_time as number).toFixed(1)}s
                  </div>
                )}
              </div>
            )}
            ...
            {lastAssistantMessage?.sources && lastAssistantMessage.sources.length > 0 && (
              <div>
                <div className="text-xs text-gray-500 mb-2 uppercase tracking-wider">
                  Fonti ({lastAssistantMessage.sources.length})
                </div>
            ...
            {!lastAssistantMessage && (
              <div className="text-center text-gray-600 text-xs pt-8">
                Le fonti e il query plan appariranno qui dopo la prima risposta.
              </div>
            )}
          </div>
        </div>
