...
- `report_generator.py` - Daily/weekly report generation (~2700 lines)
  ...
  - **Romania Vertical** (`report_type` parameter on `generate_report()`): `"global"` (default, unchanged), `"romania-daily"`, `"romania-weekly"`. When Romania, `generate_report()` applies Romania-specific cadence weights to article filtering, then branches into `_generate_romania_report()` after narrative context is fetched.
    - `_generate_romania_report(report_type, recent_articles, narrative_ctx, unique_rag_results, days, report_date)` — dispatches to `ROMANIA_DAILY_SYSTEM_PROMPT` or `ROMANIA_WEEKLY_SYSTEM_PROMPT` (Italian-language constants defined before the class), prepends `_format_romania_macro_header()`, builds prompt with articles + RAG + narrative XML. Returns dict with `report_type`, `is_short_form`, `relevance_signal_breakdown`. Model: T1 with `timeout=240s`, `temperature=0.35`.
    - `_format_romania_macro_header()` — queries `macro_indicators WHERE country_code='RO'` for 5 keys (BNR_RATE, RO_CPI_YOY, EUR_RON, RO_DEFICIT_GDP, RO_10Y_YIELD), formats as single-line: `"Contesto macro [YYYY-MM-DD]: BNR X% · CPI YoY X% · EUR/RON X · Deficit/PIL X% · 10Y RON X%"`. Values >30 days old shown as `n/d`.
