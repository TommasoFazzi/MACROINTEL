...
- `report_generator.py` - Daily/weekly report generation (~2700 lines)
  ...
  - **`_get_macro_indicators_for_screening(target_date)`**: Universal data fallback — if no rows found for `target_date` (weekends, NYSE holidays, or morning reports before that day's evening fetch), looks back up to 5 days for the most recent available record. Phase 3 / v2 always runs using the most recent complete session close.
  - **Phase 4 — Macro Analysis LLM call #1**: `_generate_macro_analysis_v2(macro_context_raw, jit_context_block, active_convergences, sc_signals, sc_prompt_block, metadata, target_date)` — calls T1 (`gemini-3.1-pro-preview`) with `timeout=180s` override (T1 default is 120s — overridden here to handle large JSON prompts). Includes 1 retry with 10s backoff for transient 504/network errors. Validates against `MacroAnalysisResultV2` (7 Literal-constrained regime labels + new optional fields `asset_state_map`, `causal_hypotheses`, `macro_state_narrative`), persists to `macro_regime_history` via `get_macro_regime_persistence_singleton()`. Called from `generate_report()` using `_phase3` data. Returns `{'success': True, 'result': validated.model_dump()}` or `{'success': False, 'error': ...}`.
  - **Phase 5 — Strategic Report LLM call #2 (active)**: `_generate_strategic_report(macro_analysis_json, articles, storylines_xml, target_date, data_quality_flags, macro_context_raw="")` — assembles prompt via `build_strategic_intelligence_prompt()`, fetches 60-day regime history XML, calls `gemini-2.5-flash` with `system_instruction`. `macro_context_raw` is the raw indicator text passed from `generate_report()` so call #2 can cross-check P30/MA/σ values directly. In `generate_report()`: if v2 analysis succeeded (`use_strategic_v2=True`), uses this path and produces 7-section strategic report; otherwise falls back silently to v1 5-section prompt. 4 module-level helpers: `_format_regime_history_xml`, `_build_data_quality_flags`, `_adapt_articles_for_strategic_prompt`, `_linkify_citations`. `metadata['macro_analysis']['strategic_v2']` tracks which path was used.
  - **V2 Macro Dashboard (Option B)**: `_format_macro_dashboard_v2(macro_v2, target_date)` — Python-only rendering of `MacroAnalysisResultV2` as a visual pre-report header, mirroring v1 style. Class-level maps: `_KEY_UNIT_FORMAT` (indicator key → format type), `_REGIME_EMOJI`, `_SEVERITY_PREFIX`. Helper `_fmt_v2_value(key, value)` formats `float` → `"$78.5"` / `"4.12%"` / `"1.0823"` / `"14.2"` based on unit type. Ticker bar: skips `noise` items, bolds `significant` items. Regime rendered with confidence % and top-2 drivers. Divergences sorted by severity (critical → significant → notable). SC signals filtered to medium/high confidence only. Wired into V2 path at `generate_report()` line ~2270 — prepended before the 7-section body, after the H1 title. V1 path (`_format_macro_dashboard()`) remains unchanged as fallback.
  - **Romania Vertical** (`report_type` parameter on `generate_report()`): `"global"` (default, unchanged), `"romania-daily"`, `"romania-weekly"`. When Romania, `generate_report()` applies Romania-specific cadence weights to article filtering, then branches into `_generate_romania_report()` after narrative context is fetched.
    - `_generate_romania_report(ctx: RomaniaReportContext)` — accepts a `RomaniaReportContext` dataclass (replaces 6 positional params). Dispatches to `ROMANIA_DAILY_SYSTEM_PROMPT` or `ROMANIA_WEEKLY_SYSTEM_PROMPT`, calls `_generate_romania_macro_analysis()` for structured macro header, builds prompt with articles + RAG + narrative XML. `format_recent_articles()` called with `fresh_cutoff` so articles from last 24h get `[FRESH]` tag. Model: T1 with `timeout=240s`, `temperature=0.35`.
    - `_generate_romania_macro_analysis()` — T5 (Flash-Lite) call that reads 5 RO indicators from DB (single `DISTINCT ON` query including `ma_7d`, `ma_30d`, `pct_change_7d`, `pct_change_30d`, `percentile_rank_30d` from migration 038), produces a 120-word Italian interpretive paragraph (monetary regime, inflation, fiscal risk, EUR/RON). Trend context now derived from pre-computed columns (`Δ7m`, `Δ30m`, `MA30m`, `P30: HIGH/LOW ⚠️`) instead of the old 60-day raw series query — cheaper and more consistent. Returns `_format_romania_macro_header() + "\n\n**Analisi macro:**\n" + paragraph`. Falls back to plain header on any failure — report never blocked.
    - **`BET_INDEX` removed from the prompt's `_LABELS` dict (2026-07-16)**: its only source (Stooq CSV endpoint) is permanently down — legacy `/q/l/` 404s for any symbol, the new `/q/d/l/` requires a JS proof-of-work challenge, no free alternative found on yfinance/TradingView. The fetch (`_fetch_stooq` in `openbb_service.py`) is left as-is (fails silently, RO indicator count stays 6/8) per explicit decision, but the LLM prompt no longer lists it — previously it always rendered as a dead "BET Index: n/d (nessun dato)" line every run. `src/api/routers/romania.py`'s `_RO_INDICATOR_KEYS` still includes it (harmless — the API loop only emits indicators that have DB rows, so it simply never appears in the response).
    - `RomaniaReportContext` — `@dataclass` defined at module level (before `ReportGenerator` class), encapsulates the 6 fields passed into `_generate_romania_report()`.
    - `_format_romania_macro_header()` — queries `macro_indicators LEFT JOIN macro_indicator_metadata WHERE country_code='RO'` for 5 keys (BNR_RATE, RO_CPI_YOY, EUR_RON, RO_DEFICIT_GDP, RO_10Y_YIELD), formats as single-line: `"Contesto macro [YYYY-MM-DD]: BNR X% · CPI YoY X% · EUR/RON X · Deficit/PIL X% · 10Y RON X%"`. Staleness threshold is **dynamic per `expected_frequency`** (`_FREQUENCY_STALE_DAYS` class-level dict): daily→3d, weekly→14d, monthly→75d, quarterly→120d, annual→400d. Previously hardcoded at 30d which caused all monthly FRED series (BNR_RATE, CPI, yield) to always show `n/d`.
    - `_get_source_geo_regions()` — queries `intelligence_sources` for `{name: geo_region}` mapping; used by `filter_storylines_for_report()` inside `_get_narrative_context()` for Romania report types.
    - `_get_source_cadence_weights(cadence_key)` — queries `intelligence_sources WHERE geo_region IN ('romania','cee')` for `{name: cadence_weight[cadence_key]}` flat dict; used by `filter_relevant_articles()` to boost article ordering.
    - `_get_narrative_context()` accepts `report_type` — fetches 30 storylines for Romania (vs 10 for global), applies `filter_storylines_for_report()` with Romania scoring, returns sorted + scored storylines with `romania_score` breakdown.
    - `filter_relevant_articles()` accepts `cadence_weight_key` and `source_cadence_weights` — when both provided, multiplies each article's similarity score by its source's cadence weight before sorting (0-weight sources rank last).
...
- `storyline_scoring.py` - Romania vertical storyline relevance scoring
  ...
  - `filter_storylines_for_report(storylines, report_type, source_geo_regions)` — scores, sorts, applies threshold from YAML config, returns top-N with `romania_score` key added. Falls back to top-3 global with `_fallback=True` when no storyline meets threshold.
  - `_jaccard(set_a, set_b)` — Jaccard similarity used by regional and trade_route signals
  - Signals: `_direct_signal` (Romania in GPE entities), `_regional_signal` (neighbor Jaccard), `_trade_route_signal` (Caspian/SGC/Middle Corridor Jaccard), `_source_signal` (fraction of geo_region=romania/cee sources), `_thematic_signal` (keyword match in title/summary)
