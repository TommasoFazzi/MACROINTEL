...
- `report_generator.py` - Daily/weekly report generation (~2700 lines)
  ...
  - **Phase 4 — Macro Analysis LLM call #1**: `_generate_macro_analysis_v2(macro_context_raw, jit_context_block, active_convergences, sc_signals, sc_prompt_block, metadata, target_date)` — calls T1 (`gemini-3.1-pro-preview`) with `timeout=180s` override (T1 default is 120s — overridden here to handle large JSON prompts). Includes 1 retry with 10s backoff for transient 504/network errors. Validates against `MacroAnalysisResultV2` (7 Literal-constrained regime labels), persists to `macro_regime_history` via `get_macro_regime_persistence_singleton()`. Called from `generate_report()` using `_phase3` data. Returns `{'success': True, 'result': validated.model_dump()}` or `{'success': False, 'error': ...}`.
