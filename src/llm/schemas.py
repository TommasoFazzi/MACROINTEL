...
# ─── Romania Vertical Schemas ──────────────────────────────────────────────────

class RomaniaStorylineSignal(BaseModel):
    """Relevance signal breakdown for a single scored storyline."""
    storyline_id: Optional[int] = None
    title: str
    romania_score: Dict[str, Any]


class RomaniaReportResult(BaseModel):
    """
    Output schema for Romania daily/weekly briefing generated by _generate_romania_report().
    Extends the standard report dict with Romania-specific metadata.
    """
    success: bool
    timestamp: str
    report_type: Literal["romania-daily", "romania-weekly"]
