...
| GET | `/api/v1/stories/graph` | routers/stories.py | Narrative graph (nodes + links + aggregate stats) for react-force-graph; query params: `min_edge_weight` (default 0.10), `min_momentum` (optional), **`view`** (`full` default / `slim`), **`max_nodes`** (default+max 1000), **`max_edges`** (default+max 8000); cached 1h per full parameter combination; filters isolated nodes (0 edges). Views (`v_active_storylines`, `v_storyline_graph`) include `emerging`, `active`, **and `stabilized`** storylines (migration 017). | Yes |
...
**The graph endpoint must stay bounded — edges grow quadratically:** `/api/v1/stories/graph` originally had no `LIMIT` and no time window, so its size tracked the corpus. Because edge count grows with the **square** of the node pool, the response degraded super-linearly: by Aug 2026 it was 2402 nodes / 41344 edges / **5.82 MB** (edges alone were 58% of the payload). That broke the public landing page in two independent ways — the fetch exceeded its 2500 ms budget (measured 3229 ms inside the frontend container), and the payload exceeded Next.js's hard **2 MB data-cache entry limit**, so `revalidate` never stored anything and every render refetched (~95 identical GETs in a 200-line log tail, 32 GB of NET I/O). Both failure modes were invisible: the landing's spec only required graceful degradation, which it did perfectly while showing stale static content for weeks. Nodes are now selected first (top-N by momentum), then only edges between selected nodes are read. **When adding a consumer, use `view=slim` unless it genuinely needs `summary`/`key_entities` — together 27% of the payload.**

**`_graph_cache` keys on the full parameter tuple:** adding a query param without adding it to `cache_key` would let one projection serve another's cached response — e.g. a `slim` body returned to `/dashboard`, silently stripping `summary` and `key_entities`.
