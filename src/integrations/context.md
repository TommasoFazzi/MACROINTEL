...
- `market_calendar.py` - NYSE holiday-aware scheduling utility
  ...
  - **MACRO_INDICATORS `fetch_category` field**: each indicator has a `fetch_category` key:
    ...
    - `trading_economics` — Trading Economics via cloudscraper (Cloudflare bypass). `_fetch_trading_economics(te_path, value_type)`. `value_type='policy_rate'` (BNR_RATE, 6.50%) or `'cpi_yoy'` (RO_CPI_YOY, ~1-2d lag). Falls back to OECD/FRED if cloudscraper unavailable or pattern breaks. ⚠️ Page structure may change — monitor.
    - `oecd` — OECD MEI_FIN SDMX-JSON API via `_fetch_oecd_mei_fin(measure, country)`. No API key. Fallback for BNR_RATE (IRSTCI). Accepts `country` param (ROU/DEU). Dynamic area_idx lookup.
...
- **External**:
  ...
  - `cloudscraper==1.2.71` - Cloudflare JS challenge bypass for Trading Economics
